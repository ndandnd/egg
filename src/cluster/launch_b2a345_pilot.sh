#!/bin/bash
# Guarded launcher for the B2 A3-A5 36-cell stabilization pilot.
#
...
# --list must report EXACTLY 36 cells (never hard-code silently); array is
# derived from that count with concurrency capped at %12. Writes a manifest
# under runs/b2a345_pilot/. Never `sbatch --wrap`.
set -euo pipefail
...
N="$(python experiments/run_b2a345_pilot.py --list | tail -1 | sed 's/total: \([0-9]*\) cells/\1/')"
if [[ "${N}" != "36" ]]; then
    echo "ERROR: pilot --list reports ${N} cells; expected exactly 36. Refusing to submit." >&2
    exit 1
fi
CONC=12  # bounded concurrency

JOB="$(sbatch --parsable --array="0-$((N - 1))%${CONC}" cluster/submit_b2a345_pilot.sub)"
mkdir -p runs/b2a345_pilot
MANIFEST="runs/b2a345_pilot/MANIFEST-$(date -u +%Y%m%dT%H%M%SZ).txt"
{
...
echo "[submitted] b2a345 pilot: ${N} cells as job ${JOB} (manifest: ${MANIFEST})"
squeue --jobs="${JOB}"
