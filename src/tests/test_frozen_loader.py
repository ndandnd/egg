        ...
        canonical_json_bytes(
            {
                ...
                "deadhead_fidelity": {
                    ...
                    "missing_link_policy": "fixture is already complete",
                },
                "physics": {
                    "service_energy": "Usage kWh",
                    "charging": "constant-power depot-only",
                },
            }
        )
    )
    source = root / "source.csv"
    source.write_bytes(b"Identifier,Start1\nRegular,04:45\n")
...
def test_manifest_binds_sources_without_copying_confidential_bytes(tmp_path):
    ...
    assert source_record == {
        ...
        "size_bytes": len(b"Identifier,Start1\nRegular,04:45\n"),
    }
...
def test_freeze_refuses_incomplete_deadhead_matrices(tmp_path):
    instance = synthetic_instance(seed=7, n_trips=4)
    ...
    with pytest.raises(FrozenSubsetError, match="identical arcs|incomplete"):
